...
const MASTERY_STORAGE_KEY = "japanese-study.hiragana-mastery";
const HIGH_SCORE_STORAGE_KEY = "japanese-study.hiragana-high-score";
const SPEED_DURATION_SECONDS = 60;

export function HiraganaFlashcardGame() {
  ...
  const [current, setCurrent] = useState<HiraganaCharacter | null>(null);
  ...
  const pool = useMemo(
    () => filterHiragana(selectedGroups, selectedGroups.length === HIRAGANA_GROUPS.length),
    [selectedGroups],
  );
  ...
  const loadNext = useCallback(
    (exclude?: string) => {
      ...
      const next = pickWeightedCharacter(pool, mastery, exclude);
      setCurrent(next);
      setFlipped(false);
      setFeedback(null);
      setOptions(buildQuizOptions(next, pool));
    },
    [mastery, pool],
  );
  ...
  function selectAllGroups() {
    setSelectedGroups(HIRAGANA_GROUPS.map((group) => group.id));
  }
  ...
  function handleQuizPick(option: HiraganaCharacter) {
    if (feedback) return;
